 ...
 */
public class CrudOpLogMaker implements IOpLogMaker {
    ...
    public OpLog makeOpLog(HttpServletRequest request, JoinPoint joinPoint, String description, String remark, String displayTitle, String displayItemFromDataStr, String displayItemFromParameter){
        ...
        if (ValidatorUtil.isNotNull(displayItemFromDataStr) && ValidatorUtil.isNotNull(dataStr)){//从dataStr中获取字段
            if (!OperationEnum.delete.getCode().equals(oper)){
                String[] names = displayItemFromDataStr.split(",");
                ...
                for (int i=0; i<names.length; i++){
                    ...
                    oprationItem += jsonObject.getString(names[i]);
                    if (i<names.length-1){
                        oprationItem += "，";
                    }
                }
                ...
                if (jsonArray.size() == 1){
                    ...
                    oprationItem = jsonArray.size() + "项";
                }
            }
        }else if (ValidatorUtil.isNotNull(displayItemFromParameter)){//从请求参数中获取字段
