...
!function($) {
  ...
  var createFormAvd = function(pColumns, searchText, that) {
    ...
    for (var i in pColumns) {
      ...
      console.log(vObjCol)
      if (vObjCol.searchcol&&!vObjCol.checkbox && vObjCol.visible && vObjCol.searchable) {
        htmlForm.push('<div class="form-group">');
        htmlForm.push(sprintf('<label class="col-sm-4 control-label">%s</label>', vObjCol.title));
        htmlForm.push('<div class="col-sm-6">');
        if(vObjCol.searchtype=='date'){
          htmlForm.push(sprintf('<input type="text" data-type="date" class="form-control input-md" name="%s" placeholder="%s" id="%s">', vObjCol.field, vObjCol.title, vObjCol.field));
        }else{
  ...
  BootstrapTable.prototype.initToolbar = function() {
    ...
    that.$toolbar.find('button[name="advancedSearch"]')
      ...
      showAvdSearch(that.columns, "条件搜索", "关闭", that);
    });
  };
